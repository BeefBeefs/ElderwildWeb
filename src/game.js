...
export class Game{
  ...
  load(){try{const raw=localStorage.getItem(SAVE_KEY);if(!raw)return freshState();const saved=JSON.parse(raw);const base=freshState();return{...base,...saved,skills:{...base.skills,...saved.skills},inventory:Array.isArray(saved.inventory)?saved.inventory.map(normalizeEntry).filter(Boolean):[],slotCapacity:Math.max(STARTING_SLOTS,saved.slotCapacity||STARTING_SLOTS),equipment:{...base.equipment,...saved.equipment},activity:null}}catch{return freshState()}}
